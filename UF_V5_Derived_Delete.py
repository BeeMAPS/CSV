...
rowcapture_list = []
QA_QC_List = []
ObservationID_Delete = []

def timecheck2(rowcapturelist):
    pinList = []
...
def timecheck(rowcapturelist):
    sorted(rowcapturelist, key=lambda observationid: observationid[11])
    rowcount = 0
    lastPin = ''
    lastObservationID = ''
    lastDate = 'Mon Dec 11 12:12:12'
    lastObservationDate = 'Mon Dec 11 23:59:59'
    ...
    for row in rowcapture_list:
        ...
        #if type(lastdatetime_Observation) is StringType:
        print ObservationID_Final
        if str(PinID_Final) == str(lastPin):
            print "same pin id"
            datetime_Observation = datetime.strptime(ObservationDate, '%a %b %d %X')
            lastdatetime_Observation = datetime.strptime(lastObservationDate, '%a %b %d %X')
            timedif = datetime_Observation - lastdatetime_Observation
            print timedif
            print "current pin id" + str(PinID_Final)
            print "last pin " + str(lastPin)
        # timedif = str(timedif)
        # timedif = timedif.split(":")[1]
        # timedifmin = timedif[-1]
            if timedif.total_seconds() <= 300 and str(timedif)[0] != '-':
                print "current observation ID: " + ObservationID_Final
                print "last observation id: " + str(lastObservationID)
                print ObservationDate
                print "last observation date" + str(lastdatetime_Observation)
                ObservationID_Delete.append(lastObservationID)
        else:
            lastObservationDate = 'Mon Dec 11 23:59:59'
        lastPin = PinID_Final
        lastObservationID = ObservationID_Final
        lastObservationDate = ObservationDate
        rowcount += 1
        # if rowcount > 15:
        #     sys.exit()
    print ObservationID_Delete
...
def QAQC(rowcaptureList):
    ...
    for row in rowcaptureList:
        ...
        # if ObservationComments_Final is "null":
        #     print "null found"
        #     Delete = 'Yes'
...
print "Checking Times out"
timecheck(rowcapture_list)
print "QA / QC"
